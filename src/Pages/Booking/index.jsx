...
export default function Booking(props) {
    ...
    const renderListChair = () => {
        return movie.danhSachGhe?.map((chair, index) => {
            return (
                <div className="chair__item" key={index}>
    ...
    const renderChair = (bookingChair, type, chair) => {
        if (type === "Thuong") {
            if (bookingChair) {
                return (
                    <button className="chair bookingChair" disabled>
                        <div className="sttChair">x</div>
                    </button>
                );
            } else {
                let cssBookingChair = "";
                ...
                return (
                    <button
                        ...
                        className={`chair ${cssBookingChair}`}
                    >
                        <div className="sttChair">{chair.stt}</div>
                    </button>
                );
            }
        } else {
            if (bookingChair) {
                return (
                    <button className="vipChair bookingVipChair" disabled>
                        <div className="sttChair">x</div>
                    </button>
                );
            } else {
                ...
                return (
                    <button
                        ...
                        className={`vipChair ${cssBookingChair}`}
                    >
                        <div className="sttChair">{chair.stt}</div>
                    </button>
                );
    ...
    const bookChair = (chair) => {
        ...
        if (listChair.length === 0) {
            ...
            if (
                chair.stt - Math.max(...arr) > 1 ||
                chair.stt - Math.min(...arr) < -1
            ) {
                Swal.fire({
                    icon: "warning",
    ...
    const renderMovie = () => {
        return (
            <div>
                ...
                <p className="border">
                    Khởi chiếu: {movie.thongTinPhim?.ngayChieu} -{" "}
                    {movie.thongTinPhim?.gioChieu} - {movie.thongTinPhim?.tenRap}
                </p>
                {listChair.length > 0 ? (
                ...
                <div className="headerRender">
                    <div className="headerLeft">
                        ...
                        <div className="content">
                            ...
                            <p>
                                Ngày chiếu: <span>{movie.thongTinPhim?.ngayChieu}</span>
                            </p>
                            <p>
                                Giờ chiếu: <span>{movie.thongTinPhim?.gioChieu}</span>
                            </p>
                        </div>
                    </div>
