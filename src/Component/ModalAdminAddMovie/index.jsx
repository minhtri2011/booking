...
export default function ModalAdminAddMovie(props) {
    const { openModal, setOpenModal, setReload, reloadState } = props;
    ...
    //thêm phim
    const handleAddMovie = (movie, resetForm) => {
        ...
        let hinhAnh = files[0];
        let movieAdd = { ...movie, maNhom: groupID, maPhim: maPhim, danhGia: danhGia, ngayKhoiChieu: ngayKhoiChieu, hinhAnh: hinhAnh };
        for (const key in movieAdd) {
            form_data.append(key, movieAdd[key]);
        }
        movieServices.addMovie(form_data).then(res => {
            resetForm();
            setFiles();
        ...
        <ModalPopup openModal={openModal} setOpenModal={setOpenModal} >
            <div className="modalAddMovieForm">
                ...
                <form onSubmit={formik.handleSubmit} id='addMovieForm'>
                    ...
                    <TextField onChange={formik.handleChange} value={formik.values.ngayKhoiChieu} InputLabelProps={{ shrink: true, }} label="Ngày khởi chiếu" type="date" name='ngayKhoiChieu' className='ngayKhoiChieu' variant="outlined" />
